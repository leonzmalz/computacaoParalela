...
void imprimirMatriz(int** m, int my_rank, int p, MPI_Comm comm);

void Read_matrix(int ** m, int my_rank, int p, MPI_Comm comm);
...
int main(int argc, char* argv[]) {
   ...
   MPI_Bcast(&TAM, 1, MPI_INT, 0, comm);
  
   if (my_rank == 0) 
      Read_matrix(mAdjacente, my_rank, p, comm);
   if (my_rank == 0) printf("We got\n");
      imprimirMatriz(mAjacente, my_rank, p, comm)
...
void Read_matrix(int ** m, int my_rank, int p, MPI_Comm comm) { 
   ...
   if (my_rank == 0) {
      temp_mat = getNovaMatriz();
      srand( (unsigned)time(NULL));
      for (i = 0; i < TAM; i++)
         for (j = 0; j < TAM; j++)
            temp_mat = rand() % 2;
      MPI_Scatter(temp_mat, TAM*TAM/p, MPI_INT, 
                  m, TAM*TAM/p, MPI_INT, 0, comm);
      free(temp_mat);
   } else {
      MPI_Scatter(temp_mat, TAM*TAM/p, MPI_INT, 
                  m, TAM*TAM/p, MPI_INT, 0, comm);
